...
class RippleProps: ObservableObject {
  @Published var imageSrc: String? = "palm_tree"
...
struct RippleView: View {
    @ObservedObject var props: RippleProps

    var body: some View {
        VStack {
            Spacer()

            Image(props.imageSrc ?? "palm_tree")
                .resizable()
                .aspectRatio(contentMode: .fit)
                .clipShape(RoundedRectangle(cornerRadius: 24))
                .onPressingChanged { point in
                ...
                .modifier(RippleEffect(at: props.origin, trigger: props.counter))
...
@objc
class RippleViewProvider: UIViewController, SwiftUIProvider {
    private var props = RippleProps()
    private var swiftUI: RippleView?
    ...
    func updateData(data: NSDictionary) {
        ...
        if (self.swiftUI == nil) {
            swiftUI = RippleView(props: props)
